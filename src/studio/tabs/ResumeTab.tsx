...
export function ResumeTab({ showToast }: ResumeTabProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast('New resume PDF uploaded to Supabase Storage!', 'success');
    } catch (err: any) {
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-[#1F2937] pb-5">
        <div>
          ...
          <p className="text-xs text-[#94A3B8]">
            Upload new PDF revisions and dynamically switch the active resume served to recruiters and visitors.
          </p>
        </div>
